...
public class EgovBBSAttributeManageServiceImpl extends EgovAbstractServiceImpl implements EgovBBSAttributeManageService {
    ...
    public Map<String, Object> BbsList(Bbs searchVO) throws Exception {
    	List<Bbs> result = attrbMngDAO.BbsList(searchVO);
    	int cnt = attrbMngDAO.BbsListCnt(searchVO);
    	Map<String, Object> map = new HashMap<String, Object>();
	...
	@Override
	public void BbsInsert(Bbs bbs) throws Exception {
		// TODO Auto-generated method stub
		
		int idx=attrbMngDAO.BbsInsert(bbs);
		
		bbs.setBbs_Idx(idx);
		
	
	
			for(int i=0;i<bbs.getBbs_Check().length;i++)
			{
				bbs.setBbs_CheckInput(bbs.getBbs_Check()[i]);
				
				attrbMngDAO.BbsCheck(bbs);
			}
	
	}

	@Override
	public void BbsUpdt(Bbs bbs) throws Exception {
		// TODO Auto-generated method stub
		attrbMngDAO.BbsUpdt(bbs);
		attrbMngDAO.BbsCheckDelete(bbs);
